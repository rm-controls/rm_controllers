...
namespace rm_chassis_controllers {
    ...
    void BalanceController::raw(const ros::Duration &period) {
        ...
        x_ref_(3) = ramp_x->output();
        x_ref_(5) = ramp_w->output();
        if (fabs(x_ref_(3)) >= 0.1)
            x_ref_(2) = x_pos_;
    }
    ...
    geometry_msgs::Twist BalanceController::iKine(const ros::Duration &period) {
        ...
        x_pos_ += period.toSec() * vel_data.linear.x;
        return vel_data;
    }
    ...
    void BalanceController::moveJoint(const ros::Duration &period) {
        ...
        x_ << pitch, imu_data_.angular_velocity.y, x_pos_, vel_data_.linear.x
                                                           + imu_data_.angular_velocity.y *
                                                             0.147, yaw, imu_data_.angular_velocity.z;
